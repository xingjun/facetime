...
#define DELTA 0x9e3779b9 /* sqr(5)-1 * 2^31 */
    ...
    */
    ...
    bool TEAEncrypt_(BYTE* pInBuffer, int nInBufferLen, BYTE* pOutBuffer, int* pnOutBufferLen, BYTE pKey[16]/* = DEF_KEY*/, UINT uRounds/* = 16*/)
    {
    ...
    bool TEADecrypt_(BYTE* pInBuffer, int nInBufferLen, BYTE* pOutBuffer, int* pnOutBufferLen, BYTE pKey[16]/* = DEF_KEY*/, UINT uRounds/* = 16*/)
    {
...
#define PIANYI 3
	bool StreamDecrypt_(const char *inbuf,int inbuflen,char *outbuf,int *outbuflen,char key[16],int type)
	{
		...
		{
			int ret = XTEADecrypt((char*)(inbuf + PIANYI),inbuflen - PIANYI,outbuf2,outbuflen2,key);
			if(ret == 0)
				return false;
			else
				outbuflen2 = ret;
	...
	bool StreamEncrypt_(const char *inbuf,int inbuflen,char *outbuf,int *outbuflen,char key[16],int type)
	{
		...
		{
			int ret = XTEAEncrypt((char*)(inbuf + PIANYI),inbuflen - PIANYI,outbuf2,outbuflen2,key);
			if(ret == 0)
				return false;
			else
				outbuflen2 = ret;
